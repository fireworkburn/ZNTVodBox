...
import com.way.app.PreferenceUtils;
...
public class FtpServerService extends Service implements Runnable {
	...
	private boolean loadSettings() {
		...
		validateBlock: {
			...
			Defaults.chrootDir = chrootDirAsFile.getAbsolutePath();
			return true;
		}
	...
	public void run() {
		...
		if (!loadSettings()) {
			...
			Log.e("","ftp server fail to start");
			return;
		}
		...
		while (!mShouldExit) {
			...
			if (mWifiListener == null) {
				...
				// crashed,
				// so spawn it
				mWifiListener = new TcpListener(mListenSocket, this);
				mWifiListener.start();
			}
		}
		try {
	...
	public static InetAddress getLocalInetAddress() {
		...
		if (isConnectedUsingWifi()) {
			Context context = FtpServerApp.getInstance();
