...
import com.znt.speaker.datastore.LocalConfigSharePreference;
...
public class DlnaUtils {
	...
	public static String creat12BitUUID(Context context){
		...
		int retryTime = 10;
		while(TextUtils.isEmpty(ip) && retryTime > 0)
		{
			retryTime --;
			ip = SystemUtils.getIP();
			MyLog.e("******************获取本地IP地址ip-->" + ip + "\n retryTime-->"+retryTime);
			try
			{
				Thread.sleep(1000);
		...
		return LocalDataEntity.newInstance(context).getDeviceInfor().getId();// + " ip:" + ip;
	}
