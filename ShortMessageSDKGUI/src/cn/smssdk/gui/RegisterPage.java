...
import cn.smssdk.gui.layout.PhoneInfo;
...
public class RegisterPage extends FakeActivity implements OnClickListener,
	...
	public void onCreate() {
		...
		if (layout != null) {
			...
			String headCode = "+" + currentCode;
			tvCountryNum.setText(headCode);

			etPhoneNum = (EditText) activity.findViewById(Res.id.et_write_phone);
			
			PhoneInfo phoneInfo = new PhoneInfo(activity);
			
			String phoneNum = phoneInfo.getNativePhoneNumber();
			if(!TextUtils.isEmpty(phoneNum) && phoneNum.startsWith(headCode))
				phoneNum = phoneNum.replace(headCode, "");
			else
				phoneNum = "";
