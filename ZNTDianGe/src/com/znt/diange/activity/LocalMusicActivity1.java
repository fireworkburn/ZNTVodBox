...
import com.znt.diange.dialog.MusicPlayDialog;
import com.znt.diange.dialog.SongAlertDialog;
import com.znt.diange.dmc.engine.OnConnectHandler;
import com.znt.diange.factory.ContentManager;
import com.znt.diange.media.MusicManager;
...
public class LocalMusicActivity1 extends BaseActivity implements OnItemClickListener, OnClickListener, IXListViewListener
{
	...
	*/
	...
	private void getMedias()
	{
		if(isLocalMusic)
		{
			ViewUtils.sendMessage(handler, SCAN_MUSIC_FINISH, MusicManager.getInstance().getMusicInforDir());
		}
		else
		{
			isRunning = false;
	...
	*/
	...
	public void onItemClick(AdapterView<?> arg0, View v, int position, long arg3)
	{
		...
		{
			if(isLocalMusic)
				ViewUtils.sendMessage(handler, SCAN_MUSIC_FINISH, MusicManager.getInstance().getMusicInforList(item.getMediaUrl()));
			else
				getMediaList(item.getMediaUrl());
			showTitle(item.getMediaName());
		}
		else
		{
			if(isLocalMusic)
			{
				//item.setMediaType(MediaInfor.MEDIA_TYPE_PHONE);
				//showSongSearchDialog(getActivity(), "本地歌曲提示", item.getMediaName(), "本地歌曲需要在线搜索，确定搜索该歌曲吗？");
				
				if(!TextUtils.isEmpty(item.getMediaName()))
				{
					Bundle bundle = new Bundle();
					bundle.putString("KEY_WORD", item.getMediaName());
			...
			else
			{
				...
				item.setMediaType(MediaInfor.MEDIA_TYPE_SPEAKER);
	...
	private class SortAdapter1 extends BaseAdapter
	{
		...
		public View getView(final int position, View view, ViewGroup arg2) 
		{
			...
			if (view == null) 
			{
				...
				viewHolder.ivMore = (ImageView)view.findViewById(R.id.iv_music_item_more);
