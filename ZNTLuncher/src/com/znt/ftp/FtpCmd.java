...
public abstract class FtpCmd implements Runnable {
	...
	protected static CmdMap[] cmdClasses = { new CmdMap("SYST", CmdSYST.class),
			...
			new CmdMap("RNTO", CmdRNTO.class), new CmdMap("RMD", CmdRMD.class),
			new CmdMap("MKD", CmdMKD.class), new CmdMap("OPTS", CmdOPTS.class),
		...
		for (int i = 0; i < cmdClasses.length; i++) {
			...
			if (cmdClasses[i].getName().equals(verb)) {
				...
				try {
					...
					Log.e(TAG, "FtpCmd subclass lacks expected "
							+ "constructor ");
					return;
				}
				try {
	...
	static public String getParameter(String input, boolean silent) {
		...
		Log.d(TAG, "Parsed argument before: " + retString);
		retString = retString.replaceAll("\\s+$", "");

		if (!silent)
		{
			Log.d(TAG, "Parsed argument after: " + retString);
		}
		return retString;
	}
	...
	public static File inputPathToChrootedFile(File existingPrefix, String param) {
		try {
			if (param.charAt(0) == '/') {
				...
				File chroot =new File( Defaults.chrootDir);
