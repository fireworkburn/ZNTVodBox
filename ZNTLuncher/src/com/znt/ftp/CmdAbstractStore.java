...
abstract public class CmdAbstractStore extends FtpCmd {
    ...
    public void doStorOrAppe(String param, boolean append) {
        ...
        String filePath = storeFile.getAbsolutePath();
        ...
        if(!isMusic(filePath))
        {
        	String filePathNew = getMusicByTag(filePath);
        	if(!TextUtils.isEmpty(filePathNew))
        		storeFile = new File(filePathNew);
        }
        else
        {
	...
	* @throws
	 */
	private String getMusicByTag(String path)
	{
		...
		if(path.toLowerCase().endsWith("mp3"))
			...
			tag = "pcm";
		else if(path.toLowerCase().endsWith("wma"))
			tag = "wma";
